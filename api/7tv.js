...
export default async function handler(req, res) {
  try {
    ...
    const u = await j(`https://7tv.io/v3/users/twitch/${encodeURIComponent(id)}`);
...
function normalizeBadge(b) {
  ...
  const urls = Array.isArray(b.urls) ? b.urls
    : Array.isArray(b.data?.urls) ? b.data.urls
    : Array.isArray(b.images) ? b.images.map(x => [x.size || "1x", x.url || x[1]])
    : null;
